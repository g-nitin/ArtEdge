...
class VGG_loss(nn.Module):
    def __init__(self, config, vgg):
        ...
        vgg = nn.Sequential(
            *list(vgg.children())[:43]
        )  # depends on what layers you want to load
    ...
    def perceptual_loss(self, content, style, trs_img):
        ...
        content = content * torch.from_numpy(np.array((0.229, 0.224, 0.225))).to(
            content.device
        ) + torch.from_numpy(np.array((0.485, 0.456, 0.406))).to(content.device)
        style = style * torch.from_numpy(np.array((0.229, 0.224, 0.225))).to(
            style.device
        ) + torch.from_numpy(np.array((0.485, 0.456, 0.406))).to(style.device)
        trs_img = trs_img * torch.from_numpy(np.array((0.229, 0.224, 0.225))).to(
            trs_img.device
        ) + torch.from_numpy(np.array((0.485, 0.456, 0.406))).to(trs_img.device)
        ...
        loss = (
            loss_c * self.config.lambda_perc_cont
            + loss_s * self.config.lambda_perc_style
        )
            ...
            for b in range(batch):  # for each image pair
                if a != b:
                    ...
                    for i in range(0, self.n_layers):  # for each vgg layer
                        loss_s_single += self.efdm_single(
                            trs_feats_vgg[i][a].unsqueeze(0),
                            style_feats_vgg[i][b].unsqueeze(0),
                        )
