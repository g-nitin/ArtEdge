...
from vgg19 import VGG_loss, vgg
...
class AesFA(nn.Module):
    ...
    def calc_G_loss(self):
        self.G_percept, self.neg_idx = self.vgg_loss.perceptual_loss(
            self.real_A, self.real_B, self.trs_AtoB
        )
        ...
        self.G_contrast = (
            self.efdm_loss(
                self.content_B_feat,
                self.style_B_feat,
                self.content_trs_AtoB_feat,
                self.style_trs_AtoB_feat,
                self.neg_idx,
            )
            * self.lambda_const_style
        )
...
class AesFA_test(nn.Module):
    ...
    def forward(self, real_A, real_B, freq, hf_weight=1.0, lf_weight=1.0):
        with torch.no_grad():
            start = time.time()
            content_A = self.netE.forward_test(real_A, "content")
            # Get the original style features (hf, lf tuple)
            style_B_hf, style_B_lf = self.netS.forward_test(real_B, "style")

            # Scale the style features based on weights
            style_B_hf_scaled = style_B_hf * hf_weight
            style_B_lf_scaled = style_B_lf * lf_weight
            style_B_scaled = (
                style_B_hf_scaled,
                style_B_lf_scaled,
            )  # Create the scaled tuple

            if freq:
                # Pass the SCALED style features to the generator
                trs_AtoB, trs_AtoB_high, trs_AtoB_low = self.netG(
                    content_A, style_B_scaled
                )
                end = time.time()
                during = end - start
                return trs_AtoB, trs_AtoB_high, trs_AtoB_low, during
            else:
                # Pass the SCALED style features to the generator's test forward
                trs_AtoB = self.netG.forward_test(content_A, style_B_scaled)
                end = time.time()
                during = end - start
    ...
    def style_blending(self, real_A, real_B_1, real_B_2):
        with torch.no_grad():
            start = time.time()
            content_A = self.netE.forward_test(real_A, "content")
            style_B1_h = self.netS.forward_test(real_B_1, "style")[
                0
            ]  # High freq from style 1
            style_B2_l = self.netS.forward_test(real_B_2, "style")[
                1
            ]  # Low freq from style 2
            style_B = style_B1_h, style_B2_l  # Combine features

            trs_AtoB = self.netG.forward_test(
                content_A, style_B
            )  # Generate using combined features
            end = time.time()
            during = end - start
